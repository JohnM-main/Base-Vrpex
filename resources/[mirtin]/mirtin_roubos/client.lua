...
vSERVER = Tunnel.getInterface("mirtin_roubos")
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                ...
                if not delay then
                    if cfg.roubos[v.type] then
                        ...
                        if vSERVER.checkRobbery(k) then
                            ...
                            vSERVER.alertPolice(v.coords[1],v.coords[2],v.coords[3], "Roubo "..tipoRoubo, "Um Roubo em "..tipoRoubo.." ("..rouboId..") acaba de ser iniciado, intercepte os assaltantes.")

                            async(function()
                                while roubando do
                                    contadorRoubo = contadorRoubo - 1
                                    vSERVER.giveItem("dirty_money", math.random(cfg.roubos[v.type].minValue,cfg.roubos[v.type].maxValue))
                                    
                                    if GetEntityHealth(ped) <= 105 and roubando then
                                        vSERVER.alertPolice(v.coords[1],v.coords[2],v.coords[3], "Roubo "..tipoRoubo, "O Roubo em "..tipoRoubo.." ("..rouboId..") foi cancelado, um dos assaltantes morreu.")
                                    ...
                                    if contadorRoubo <= 0 and roubando then
                                        vSERVER.alertPolice(v.coords[1],v.coords[2],v.coords[3], "Roubo "..tipoRoubo, "O Roubo em "..tipoRoubo.." ("..rouboId..") foi concluido com sucesso, intercepte os assaltantes.")
                                        vSERVER.cancelRobbery()
...
RegisterCommand('croubar', function(source,args)
    if roubando then
        vSERVER.alertPolice(cfg.locationRoubos[rouboId].coords[1],cfg.locationRoubos[rouboId].coords[2],cfg.locationRoubos[rouboId].coords[3], "Roubo "..tipoRoubo, "O Roubo em "..tipoRoubo.." ("..rouboId..") foi cancelado pelos assaltantes.")
        vSERVER.cancelRobbery()
